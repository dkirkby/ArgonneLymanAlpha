...
def simple_zscan(wave,flux,ivar,template_wave,template_flux,zstep=0.001,zmin=0.,zmax=100.,wave_nsig=2.,ntrack=3,recursive=True) :
    ...
    for z in np.linspace(zmin,zmax,num=int((zmax-zmin)/zstep+1)) :
        ...
        for frame_index in range(nframes) :
            model[frame_index] = np.interp(wave[frame_index],template_wave*(1+z),template_flux)
            sum_ivar_flux_prof += np.sum(ivar[frame_index]*flux[frame_index]*model[frame_index])
            sum_ivar_prof2     += np.sum(ivar[frame_index]*model[frame_index]**2)
        ...
        if chi2<np.max(best_chi2s) :
            ...
            if tmp.size>0 : # there is an existing close redshift we replace this entry
                ...
                print "DEBUG INSERT for chi2=%f z=%f i=%d OTHERS z=%s chi2=%s"%(chi2,z,i,str(best_zs[best_chi2s<very_large_number]),str(best_chi2s[best_chi2s<very_large_number]))
    ...
    if recursive :
        log.info("first pass best z =%f chi2/ndf=%f, second z=%f dchi2=%f, third z=%f dchi2=%f"%(best_zs[0],best_chi2s[0]/ndf,best_zs[1],best_chi2s[1]-best_chi2s[0],best_zs[2],best_chi2s[2]-best_chi2s[0]))
        #pylab.plot(zz,zchi2)
        #pylab.show()
        #sys.exit(12)
...
def save() :
    ...
    output_hdulist.writeto(args.outfile,clobber=True)
...
def main() :
    ...
    for spec,q in zip(qso_spectra,range(nqso)) :
        ...
        result = simple_zscan(wave,flux,ivar,template_wave,template_flux,zstep=0.001,zmin=0.3,zmax=4.,wave_nsig=2.,ntrack=3,recursive=True)
